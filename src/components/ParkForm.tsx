...
export default function ParkForm({ initialData }: ParkFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const url = initialData?.id ? `/api/parks/${initialData.id}` : '/api/parks'
    ...
    return (
        <form onSubmit={handleSubmit} style={{ maxWidth: '800px', margin: '0 auto', background: 'white', padding: '2rem', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
            <div className="grid grid-cols-2" style={{ gap: '1rem', marginBottom: '1rem' }}>
                <div>
                    <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 'bold' }}>Nome do Parque</label>
                    <input
                        type="text" required
            ...
            <div style={{ marginBottom: '1rem' }}>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 'bold' }}>Descrição Geral</label>
                <textarea
                    required
            ...
            <div style={{ marginBottom: '1rem' }}>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 'bold' }}>Detalhes de Atrações</label>
                <textarea
                    required
                    value={formData.attractionsDetails} onChange={e => setFormData({ ...formData, attractionsDetails: e.target.value })}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', flexWrap: 'wrap' }}>
                ...
                <button type="submit" disabled={loading} className="btn btn-primary">
                    {loading ? 'Salvando...' : 'Salvar Parque'}
                </button>
            </div>
        </form>
    )
